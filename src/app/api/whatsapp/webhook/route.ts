...
function supabaseAdmin() {
  return mockAdminClient as unknown as { from: (tableName: string) => ServerMockQueryBuilder }
}
...
export async function POST(request: Request) {
  ...
  const requestUrlBase = requestUrl.origin

  // Process asynchronously so we can ack Meta within their timeout.
  processWebhook(body, requestUrlBase).catch((error) => {
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }, requestUrlBase: string) {
...
async function processMessage(
  ...
  // Automatic Order Tracking Integration
  if (message.type === 'order' && message.order) {
    try {
      // 1. Fetch existing active products for the user to match SKU
      ...
      const itemsPayload = []
      let totalAmount = 0
      const orderCurrency = message.order.product_items[0]?.currency || 'XOF'

      for (const item of message.order.product_items) {
        const skuStr = item.product_retailer_id
        const qty = Number(item.quantity) || 1
        const price = Number(item.item_price) || 0
        totalAmount += price * qty

        // Search for existing product by SKU (case insensitive)
        let product = dbProducts?.find(
          (p: any) => p.sku?.toLowerCase() === skuStr.toLowerCase()
        )

        // If product does not exist, insert it automatically
        if (!product) {
          ...
          if (newProdErr) {
            console.error('[webhook] Error auto-creating product:', newProdErr)
          } else {
            product = newProd
          }
        }
        ...
        itemsPayload.push({
          product_id: product?.id || `placeholder-${skuStr}`,
          quantity: qty,
          price,
        })
      }
  ...
  // Trigger AI Auto-reply asynchronously if not paused and there's an active agent
  void (async () => {
    try {
      ...
      const isAiPaused = convCheck?.ai_paused ?? false
      ...
      if (!isAiPaused) {
        ...
        if (activeAgents && activeAgents.length > 0) {
          ...
          const chatUrl = new URL('/api/ai/chat', requestUrlBase)
          const chatRes = await fetch(chatUrl.toString(), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              messages: (recentMsgs || []).map((m: { sender_type: string; content_text: string | null }) => ({
          ...
          if (chatRes.ok) {
            ...
            if (answer) {
              ...
              const { error: botMsgErr } = await supabaseAdmin()
                .from('messages')
                .insert({
                  conversation_id: conversation.id,
                  sender_type: 'bot',
              ...
              if (!botMsgErr) {
                ...
                console.error('Error inserting bot message:', botMsgErr)
              }
            }
          } else {
            console.error('AI Chat API failed:', await chatRes.text())
          }
        }
      }
    } catch (err) {
      console.error('Failed to auto-reply with AI:', err)
    }
  })()
}
...
async function parseMessageContent(
  ...
  switch (message.type) {
    ...
    case 'order':
      if (message.order) {
        const orderText = message.order.text ? `\n\n"${message.order.text}"` : ''
        const itemsText = message.order.product_items
          .map((item) => {
            const price = Number(item.item_price) || 0
            const qty = Number(item.quantity) || 1
            const itemTotal = price * qty
            return `• ${qty}x ${item.product_retailer_id} (${price} ${item.currency}) = ${itemTotal} ${item.currency}`
          })
          .join('\n')

        const totalAmount = message.order.product_items.reduce(
          (acc, item) => acc + (Number(item.item_price) || 0) * (Number(item.quantity) || 1),
          0
        )
        const orderCurrency = message.order.product_items[0]?.currency || 'XOF'

        const text = `🛒 *Nouvelle commande panier* :${orderText}\n\n${itemsText}\n\n*Total :* ${totalAmount} ${orderCurrency}`
        return {
          contentText: text,
          mediaUrl: null,
          mediaType: null,
      ...
      return { contentText: '🛒 Commande reçue (Panier vide ou invalide)', mediaUrl: null, mediaType: null }
