...
const fallbackDictionary: Record<string, Record<string, string>> = {
  fr: {
    ...
    "sign out of every device where you're logged in — including this one. useful if you lost a laptop or shared your password.": "Déconnetez-vous de tous les appareils sur lesquels vous êtes connecté — y compris celui-ci. Utile si vous avez perdu un ordinateur ou partagé votre mot de passe.",
...
export async function POST(request: Request) {
  try {
    const { text, targetLang } = await request.json() as { text: string; targetLang: 'fr' | 'en' }
    if (!text || !targetLang) {
    ...
    // 1. Check fallback dictionary first (casing-insensitive matching)
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `You are a translation assistant for a WhatsApp CRM dashboard named WaPulse. Translate the following static UI text to ${targetLang === 'fr' ? 'French' : 'English'}.
Return ONLY the translation without any explanation, markdown, or extra spaces. Keep punctuation and placeholders exactly as they are. Preserve casing styles (e.g. if Title Case, output Title Case). If the text is already in the target language or is a code/name, return it exactly as is.`
            },
            {
              role: 'user',
              content: text
    ...
    // 3. Fallback: if OpenAI is not configured or failed, return the text as is.
