    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            {t("Manage your contact list.")} {totalCount > 0 && `${totalCount} ${t(totalCount === 1 ? "total contact." : "total contacts.")}`}
          </p>
        </div>
              ...
              <TableRow className="border-slate-800">
                <TableCell colSpan={7} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-slate-500">{t("Loading contacts...")}</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : contacts.length === 0 ? (
                ...
                <TableCell colSpan={7} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-slate-500">
                      {search ? t('No contacts match your search.') : t('No contacts yet.')}
                    </p>
                    {!search && (
                      <Button
                        variant="outline"
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-xs text-slate-500">
            {t("Showing")} {page * PAGE_SIZE + 1}-{Math.min((page + 1) * PAGE_SIZE, totalCount)} {t("of")}{' '}
            {totalCount}
          </p>
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 text-slate-200 sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">{t("Delete Contact")}</DialogTitle>
