...
export default function AutomationsPage() {
  ...
  async function load() {
    try {
      ...
      setError(err instanceof Error ? err.message : t("Failed to load automations"))
    }
  }
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    toast.success(next ? t("Automation activated") : t("Automation paused"))
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">{t("Automations")}</h1>
          <p className="mt-1 text-sm text-slate-400">
            {t("Build workflows that react to WhatsApp® events automatically.")}
          </p>
        </div>
        <Button
      ...
      {showTemplates && (
        <section>
          <h2 className="mb-3 text-sm font-semibold text-slate-300">{t("Quick-start templates")}</h2>
      ...
      {automations.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-slate-800 bg-slate-900/40">
          ...
          <p className="mt-3 text-sm font-medium text-white">{t("No automations yet")}</p>
          <p className="mt-1 text-xs text-slate-400">
            {t("Pick a template above or create one from scratch.")}
          </p>
        </div>
      ) : (
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="text-white">{pendingDelete?.name}</span> {t("and its execution history. This cannot be undone.")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
        ...
        >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
            ...
            <span className="tabular-nums">
              {automation.execution_count} {t(automation.execution_count === 1 ? "run" : "runs")}
            </span>
            <span aria-hidden>·</span>
            <span>{t("last")} {t(formatRelative(automation.last_executed_at))}</span>
          </div>
        </button>
