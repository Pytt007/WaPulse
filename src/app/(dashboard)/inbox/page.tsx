...
export default function InboxPage() {
  ...
  const handleAiPausedChange = useCallback(
    (conversationId: string, aiPaused: boolean) => {
      setConversations((prev) =>
        prev.map((c) =>
          c.id === conversationId
            ? { ...c, ai_paused: aiPaused }
            : c
        )
      );
      if (activeConversation?.id === conversationId) {
        setActiveConversation((prev) =>
          prev ? { ...prev, ai_paused: aiPaused } : prev
        );
      }
    },
    [activeConversation]
      ...
      {whatsappConnected === false && (
        <div className="flex shrink-0 items-center justify-center gap-2 border-b border-amber-500/20 bg-amber-500/10 px-4 py-2">
          ...
          <p className="text-xs text-amber-400">
            {t("WhatsApp® is not connected. Go to Settings to connect your account.")}
          </p>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <MessageThread
            ...
            onAiPausedChange={handleAiPausedChange}
