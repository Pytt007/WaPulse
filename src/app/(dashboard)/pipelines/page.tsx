    ...
    <div className="space-y-6">
          ...
          <DropdownMenu>
            ...
            >
              ...
              <span className="font-semibold">
                {t(selectedPipeline?.name ?? "Select Pipeline")}
              </span>
      ...
      {pipelines.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-700 py-20">
          ...
          <p className="mt-2 text-sm text-slate-400">
            {t("Create a pipeline to start tracking deals")}
          </p>
          <Button
            onClick={() => setNewPipelineOpen(true)}
      ...
      <Dialog open={newPipelineOpen} onOpenChange={setNewPipelineOpen}>
        <DialogContent className="sm:max-w-sm bg-slate-900 border-slate-700">
          <DialogHeader>
            <DialogTitle className="text-white">{t("New Pipeline")}</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <Label className="text-slate-300">{t("Pipeline Name")}</Label>
            <Input
              value={newPipelineName}
              onChange={(e) => setNewPipelineName(e.target.value)}
              placeholder={t("e.g., Enterprise Sales")}
            ...
            <p className="mt-2 text-xs text-slate-400">
              {t("Default stages (New Lead → Won) will be created automatically.")}
            </p>
          </div>
          <DialogFooter className="bg-slate-900/50 border-slate-700">
            ...
            <Button
              ...
              className="bg-violet-600 text-white hover:bg-violet-700"
            >
              {creating ? t("Creating...") : t("Create Pipeline")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
