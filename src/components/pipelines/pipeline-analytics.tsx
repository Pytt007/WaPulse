...
export function PipelineAnalytics({ stages, deals }: PipelineAnalyticsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const totalValueXOF = active.reduce((sum, d) => sum + convertCurrency(d.value || 0, d.currency || "XOF", "XOF"), 0);
    const avgValueXOF = totalCount > 0 ? totalValueXOF / totalCount : 0;

    const stageById = new Map(sortedStages.map((s) => [s.id, s]));
    const weightedValueXOF = openDeals.reduce((sum, d) => {
      const stage = stageById.get(d.stage_id);
      if (!stage) return sum;
      const prob = computeStageProbability(stage, sortedStages);
      return sum + convertCurrency(d.value || 0, d.currency || "XOF", "XOF") * prob;
    }, 0);
