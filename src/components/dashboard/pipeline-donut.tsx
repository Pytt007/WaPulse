  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-slate-800 bg-slate-900">
      <header className="border-b border-slate-800 px-5 py-4">
        <h2 className="text-sm font-semibold text-white">{t("Pipeline Value")}</h2>
        <p className="mt-0.5 text-xs text-slate-500">
          {t("Open deals by stage")}
        </p>
      </header>
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <EmptyState
            ...
            hint={t("Create deals in pipelines to see stage breakdowns here.")}
          />
        ) : (
          <>
            <Donut data={data} />
              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-slate-500 tabular-nums">
                    {s.dealCount} {s.dealCount === 1 ? t("deal").toLowerCase() : t("deals").toLowerCase()}
                  </span>
                  <span className="w-20 text-right text-slate-300 tabular-nums">
                    {formatPrice(s.totalValue, 'XOF', { notation: 'compact', maximumFractionDigits: 2 })}
                  </span>
                </li>
              ))}
            </ul>
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label="Pipeline value by stage">
        ...
        >
          {formatPrice(data.totalValue, 'XOF', { notation: 'compact', maximumFractionDigits: 2 })}
        </text>
      </svg>
    </div>
  )
