    ...
    <section className="flex h-full flex-col rounded-xl border border-slate-800 bg-slate-900">
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <LineSvg data={data} maxY={maxY} ticks={niceTicks} language={language} />
        )}
      </div>
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      {hovered && hover !== null && (
        ...
        >
          <div className="font-medium text-white">{longDayLabel(hovered.day, language)}</div>
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-blue-300">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-blue-500" />
              {hovered.incoming} {t("incoming").toLowerCase()}
            </span>
            <span className="flex items-center gap-1.5 text-violet-300">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-violet-500" />
              {hovered.outgoing} {t("outgoing").toLowerCase()}
            </span>
          </div>
        </div>
      )}
