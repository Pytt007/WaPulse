...
export function PasswordForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile?.email) {
      toast.error(t('Cannot change password without a current email'));
      return;
    }
    if (next.length < MIN_PASSWORD) {
      setConfirmError(t('Password must be at least') + ` ${MIN_PASSWORD} ` + t('characters'));
      return;
    }
    if (next !== confirm) {
      setConfirmError(t('New password and confirmation do not match'));
      return;
    }
    ...
    <Card className="bg-slate-900/40 border-slate-800">
      <CardHeader>
        ...
        <CardDescription className="text-slate-400">
          {t("Use at least")} {MIN_PASSWORD} {t("characters. You will stay signed in on this device after changing it.")}
        </CardDescription>
      </CardHeader>
