...
export function ProfileForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        emailSent
          ? t('Profile saved — check your email to confirm the address change')
          : t('Profile saved'),
      );
    } catch (err) {
    ...
    <Card className="bg-slate-900/40 border-slate-800">
      <CardHeader>
        ...
        <CardDescription className="text-slate-400">
          {t("How you show up across the app. Your avatar and name appear in the header, sidebar, and anywhere your teammates see you.")}
        </CardDescription>
      </CardHeader>
          ...
          <div className="flex flex-wrap items-center gap-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {currentAvatar ? t('Change photo') : t('Upload photo')}
              </Button>
              {currentAvatar && (
                <Button
                  type="button"
              ...
              <p className="w-full text-xs text-slate-500">
                {t("PNG, JPG, WebP, or GIF. Up to 2 MB.")}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {emailChangePending && (
              <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-amber-300">
                ...
                <span>
                  {t("Check the inbox for")} <strong>{profile?.email}</strong> {t("and")}{' '}
                  <strong>{email}</strong> {t("— both need to confirm before the change takes effect.")}
                </span>
              </p>
            )}
          </div>
            ...
            <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              <div>
                ...
                <dd className="mt-0.5 font-mono text-slate-200">
                  {profile?.role ? t(profile.role) : t('user')}
                </dd>
              </div>
              <div>
                <dt className="text-slate-500">{t("Joined")}</dt>
                <dd className="mt-0.5 text-slate-200">{joined}</dd>
              </div>
              <div className="sm:col-span-2">
                <dt className="text-slate-500">{t("User ID")}</dt>
