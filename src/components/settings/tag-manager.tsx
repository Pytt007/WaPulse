  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-white">{t("Tags")}</h2>
          <p className="text-sm text-slate-400">{t("Organize your contacts with color-coded tags.")}</p>
        </div>
        <Button
          onClick={() => {
            setNewTagName('');
      ...
      {tags.length === 0 ? (
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            <p className="text-slate-400 text-sm">{t("No tags yet.")}</p>
            <p className="text-slate-500 text-xs mt-1">{t("Create tags to categorize your contacts.")}</p>
          </CardContent>
        </Card>
      ) : (
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">{t("New Tag")}</DialogTitle>
            <DialogDescription className="text-slate-400">
              {t("Create a new tag with a name and color.")}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label className="text-slate-300">{t("Tag Name")}</Label>
              <Input
            ...
            {/* Preview */}
            <div className="space-y-2">
              <Label className="text-slate-300">{t("Preview")}</Label>
              <div>
                <span
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">{t("Delete Tag")}</DialogTitle>
            <DialogDescription className="text-slate-400">
              {t("Are you sure you want to delete the tag")} &quot;{tagToDelete?.name}&quot;? {t("This will remove it from all contacts. This action cannot be undone.")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="bg-slate-900 border-slate-700">
            <Button
