...
interface MessageThreadProps {
  ...
  onAiPausedChange?: (conversationId: string, aiPaused: boolean) => void;
  /**
...
function formatDateSeparator(dateStr: string, t: (s: string) => string): string {
...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    const remaining =
      hoursLeft >= 1
        ? `${Math.floor(hoursLeft)}h ` + t("remaining")
        : `${Math.floor(hoursLeft * 60)}m ` + t("remaining");
  ...
  const handleSend = useCallback(
    async (
      ...
      const optimisticMsg: Message = {
        ...
        content_text: text || (media?.type === "document" ? media.filename : undefined),
      ...
      try {
        const res = await fetch("/api/whatsapp/send", {
          ...
          body: JSON.stringify({
            ...
            content_text: text || (media?.type === "document" ? media.filename : null),
            media_url: media?.url,
            reply_to_message_id: replyToId,
          }),
        });
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error(t("Wait for the message to finish sending"));
        return;
      }
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : "network error";
        toast.error(`${t("Reaction failed")}: ${reason}`);
    ...
    <div className="flex flex-1 flex-col bg-slate-950">
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-800 bg-slate-900 px-3 py-3 sm:px-4">
        ...
        <div className="flex items-center gap-2">
          {/* AI Toggle Switch */}
          <div className="flex items-center gap-2 rounded-md border border-slate-800 bg-slate-950/40 px-2 py-1 select-none">
            <div className="relative flex items-center justify-center">
              <Bot className={cn(
                "h-4 w-4 transition-colors",
                conversation.ai_paused ? "text-slate-500" : "text-violet-400"
              )} />
            ...
            <span className={cn(
              ...
              conversation.ai_paused ? "text-slate-500" : "text-violet-200"
            )}>
              {conversation.ai_paused ? "IA en pause" : "IA active"}
            </span>
            <Switch
              checked={!conversation.ai_paused}
              onCheckedChange={async (checked) => {
                const isPaused = !checked;
                const supabase = createClient();
                const { error } = await supabase
                  .from("conversations")
                ...
                if (error) {
                  ...
                  toast.error(t("Failed to update AI status"));
                  return;
                }
                
                onAiPausedChange?.(conversation.id, isPaused);
                toast.success(isPaused ? t("AI agent has been paused") : t("AI agent is now active"));
              }}
            />
          </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-slate-500">{t("No messages yet")}</p>
            <p className="text-xs text-slate-600">
              {t("Send a template to start the conversation")}
            </p>
          </div>
        ) : (
